...
You MUST split the code into the following structure:
1.  `CommandDispatcher.h`: Header guards, `ROME` namespace, Singleton declaration, `Command` struct, and public interface.
2.  `CommandDispatcher.cpp`: Implementation of the Singleton, `Push()`, `Pop()`, and `Process()` loop.
3.  `CommandHooks.h/cpp`: RE::Console and RE::Actor execution logic (ExecuteRaw, ExecuteKill).
4.  `JsonProcessor.h/cpp`: Integration with `nlohmann::json` for command parsing.
