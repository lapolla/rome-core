#!/bin/bash
# ROME V6: START PEER MESH
ROME_ROOT="$(cd "$(dirname "$0")" && pwd)"
WS_URL="ws://127.0.0.1:8741"
WS_TOKEN="ROME_V4_SECURE_TOKEN"

# 0. Kill stale workers and daemon
pkill -f "dist/legion_worker.js" 2>/dev/null || true
pkill -f "dist/peer_server.js" 2>/dev/null || true
sleep 1

# 1. Start Peer Server (Daemon)
echo "Starting ROME Peer Server (Port 8741)..."
nohup node "$ROME_ROOT/dist/peer_server.js" DAEMON 8741 > "/tmp/rome-daemon.log" 2>&1 &
sleep 2
...
start_worker() {
    ...
    nohup node "$ROME_ROOT/dist/legion_worker.js" \
        --mode worker \
        --capabilities "$cap" \
        --ws-url "$WS_URL" \
        --ws-token "$WS_TOKEN" \
