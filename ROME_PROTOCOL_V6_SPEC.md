...
## 3. Real-Time Interaction Loop (The "V6 Pulse")
...
V6 removes the distinction between "parent" and "child" orchestrators. Every PeerServer can handle recursive dispatches. Agents can build dynamic DAGs at runtime based on the feedback they receive from the system.

## 5. Security & Safety (DSA v2)
Direct Shell Access (DSA) in v6 is gated by the `SAFE_SHELL` capability. Agents must explicitly use the `[ROME_SHELL: ...]` or `[ROME_DISPATCH: SAFE_SHELL ...]` signals, which are audited by the PeerServer.
